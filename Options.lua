...
local customSpellInfo = {
    ...
    addChild = {
        ...
        desc = "Add a child spell ID to this spell. Child IDs will be checked like normal IDs but will use the same icon as their parent. Also, any changes to the parent will also apply to all its children. This is useful for spells that have multiple ids which are convenient to track as a single spell (e.g. different ranks of the same spell).",
        width = 1,
        validate = function(_, value)
            local num = tonumber(value)
